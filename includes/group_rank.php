...
// rank for winners cup finals
function rank1($teams, $matches) {
    // compute the information for each team, e.g. win, draw, etc.
    foreach ($teams as $team_name=>$team) {
    	$win = 0;
    	$draw = 0;
...
function compare_two_teams($matches, $teams, $index1, $index2) {
	...
	// if swap is required
	if ($score2 > $score1 || ($score1 == $score2 && $away_score2 > $away_score1)) {
...
// get chinese name of the team
function get_team_chinese_name($conn, $team_name) {
