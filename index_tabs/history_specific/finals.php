						<?php
						// start a new finals
        				if ($match_index == 0) $prev_match = array("tournament"=>"", "competition"=>"", "group"=>"", "round"=>"");
        				else $prev_match = $matches[$match_index - 1];
        				if ($tournament != $prev_match["tournament"] || $competition != $prev_match["competition"] || $group != $prev_match["group"] || $round != $prev_match["round"]) {
        					echo '
        		<div class="col-lg-60 col-120 border-div" style="margin-top: 10px; padding-bottom: 7px;">
        			<p style="font-size: 16px; font-weight: bold;">
        				<a href="competition.php?tournament='.$tournament.'&competition='.$competition.'&stage=knockout&round=finals" target="_blank" style="text-decoration: underline; color: black;">
        					第 '.$competition.' 届 '.$tournament_chinese.' 联合决赛
        				</a>
        			</p>
        			<div class="row">
        				<div class="col-xxl-56 col-xl-80 col-lg-100 col-md-56 col-120">
	        				<a href="competition.php?tournament='.$tournament.'&competition='.$competition.'&stage=group&group='.$group.'" target="_blank" style="text-decoration: none; color: black;">
	                        ...
	                        $group_matches = array();
			                $sql = 'SELECT * FROM winners_cup WHERE competition='.$match["competition"].' AND round="finals"';
			                ...
			                foreach ($teams as $team_index => $team) {
			                    ...
			                    echo '
	                                <tr style="font-weight: '.$font_weight.';">
	                                    <td style="color: '.$color.';">&nbsp;<img src="images/teams/'.$team_name.'.png" style="width: 20px; height: 20px; margin-top: 3px;"> '.$team_chinese_name.'</td>
        				...
        				if ($tournament != $next_match["tournament"] || $competition != $next_match["competition"] || $group != $next_match["group"]) {
        					...
        					foreach ($temp_matches as $match) {
        						...
        						if ($size == 6) $class = "";
        						echo '
        						<div class="col-sm-20 col-120'.$class.'" style="padding-right: 0px;">第 '.($match["game"]+1).' 轮</div>
