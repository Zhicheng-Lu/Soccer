<?php
function bipartite($positions, $current_position, $teams, $tournament, $number_of_groups_with_two_nation_teams) {
	for ($j=0; $j < sizeof($teams); $j++) {
		if ($teams[$j]["availability"] == "available") {
			if (!check_valid($positions[$current_position], $teams[$j], $tournament, $number_of_groups_with_two_nation_teams)) {
...
function build_graph($positions, $current_position, $teams, $temp_team, $tournament, $number_of_groups_with_two_nation_teams) {
	$s = new Node();
	$s->id = "s";
	$t = new Node();
	$t->id = "t";
	...
	for ($j=0; $j < sizeof($teams); $j++) { 
		if (($teams[$j]["availability"] == "available" || $teams[$j]["availability"] == "infeasible") && $j != $temp_team) {
			...
			for ($i=$current_position+1; $i < sizeof($positions); $i++) {
				if (check_valid($positions[$i], $teams[$j], $tournament, $number_of_groups_with_two_nation_teams)) {
					$edge = new Edge();
